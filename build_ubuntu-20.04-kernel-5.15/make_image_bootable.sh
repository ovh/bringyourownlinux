...
finalize() {
    ...
    # another system, so refresh it with a fresh one before update-initramfs so
    # the initramfs embeds the new hostid. zgenhostid's -f flag only exists in
    # ZFS >= 2.1 (focal ships 0.8), and without it zgenhostid exits non-zero
    # when /etc/hostid already exists; remove the file first so a fresh random
    # id is generated on any ZFS version.
